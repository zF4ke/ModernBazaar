        ...
        <div className="divide-y divide-border/50">
          {items.map((o, i) => {
            ...
            return (
              ...
              >
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-muted font-mono text-sm font-semibold text-muted-foreground">
                  {i + 1}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="truncate font-medium">{o.displayName || o.productId}</div>
                  <div className="mt-0.5 flex items-center gap-1.5 text-xs text-muted-foreground">
                    <span>Buy <span className="font-mono">{compact(buy)}</span></span>
                    <ChevronRight className="h-3 w-3 text-muted-foreground/50" />
                    <span>Sell <span className="font-mono">{compact(sell)}</span></span>
                    <span className="ml-2"><span className="font-mono">{pct(o.spreadPct)}</span> spread</span>
                  </div>
                </div>
                <div className="text-right">
                  <div className="font-mono text-sm font-semibold text-gain">+{compact(o.reasonableProfitPerHour)}</div>
